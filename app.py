...
# Function to check OpenAI usage and credits
def check_openai_usage():
    ...
    if response.status_code == 200:
        ...
        total_usage = usage_data.get("total_usage", 0)  # Total usage in tokens or dollars
        return total_usage
    else:
        st.error("Failed to retrieve usage data from OpenAI API.")
        return None

# Display remaining credits on the Streamlit app
def display_remaining_credits():
    usage = check_openai_usage()
    
    if usage is not None:
        st.write(f"Total usage: {usage}")
        # Add logic here to display remaining credits or limit checks
    else:
        st.write("Unable to fetch usage data.")
